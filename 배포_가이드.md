...
**프로젝트 파일 수정** (`SpatialCheckPro.GUI.csproj`):
...
```bash
dotnet publish SpatialCheckPro.GUI/SpatialCheckPro.GUI.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true
```
...
```
SpatialCheckPro/
├── SpatialCheckPro.GUI.exe          # 단일 실행 파일 (Self-contained)
├── Config/                           # 설정 파일 디렉토리
│   ├── 1_table_check.csv
│   ├── 2_schema_check.csv
...
1. **프로젝트 파일 확인**
   - `SpatialCheckPro.GUI.csproj`에 Self-contained 설정 추가
...
```inno
[Setup]
AppName=SpatialCheckPro
AppVersion=1.0.0
DefaultDirName={pf}\SpatialCheckPro
DefaultGroupName=SpatialCheckPro
OutputDir=installer
OutputBaseFilename=SpatialCheckPro_Setup_v1.0.0
Compression=lzma2
...
[Files]
Source: "publish\SpatialCheckPro.GUI.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "publish\Config\*"; DestDir: "{app}\Config"; Flags: ignoreversion recursesubdirs

[Icons]
Name: "{group}\SpatialCheckPro"; Filename: "{app}\SpatialCheckPro.GUI.exe"
Name: "{commondesktop}\SpatialCheckPro"; Filename: "{app}\SpatialCheckPro.GUI.exe"

[Run]
Filename: "{app}\SpatialCheckPro.GUI.exe"; Description: "SpatialCheckPro 실행"; Flags: nowait postinstall skipifsilent
```
