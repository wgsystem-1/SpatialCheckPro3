...
## 1. 개요
현재 SpatialCheckPro의 검수 규칙 ID(`RuleId`)는 한글 명칭(예: `건물_도로경계면_침범`), 영문 약어(예: `ATTR_CODE_BULD`), 임의 코드(예: `G26`)가 혼재되어 있어 체계적인 관리와 확장이 어렵습니다.
...
### 3.1. 설정 파일 (`SpatialCheckPro/Config/*.csv`) [영향도: 높음]
